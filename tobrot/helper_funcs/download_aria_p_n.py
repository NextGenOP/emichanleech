...
def add_magnet(aria_instance, magnetic_link, c_file_name):
    ...
    try:
        ...
        return False, "**Gagal** \n" + str(e) + " \nTolong jangan pakai link yang lambat. "
    else:
...
def add_torrent(aria_instance, torrent_file_path):
    if torrent_file_path is None:
        return False, "**Gagal** \n" + str(e) + " \nada yang salah saat menambahkan<u>TORRENT</u> file"
    if os.path.exists(torrent_file_path):
        ...
        return False, "**Gagal** \n" + str(e) + " \nMohon gunakan penyedia yang lainnya."
...
async def fake_etairporpa_call(
    ...
    if incoming_link.startswith("magnet:"):
        ...
        if err_message is not None:
            ...
            return False, "tidak dapat mengambil metadata \n\n#stopped"
    await asyncio.sleep(1)
...
async def call_apropriate_function(
    ...
    for key_f_res_se in final_response:
        ...
        message_to_send += "\n"
    if message_to_send != "":
        mention_req_user = f"<a href='tg://user?id={user_id}'>Permintaan file anda</a>\n\n"
        message_to_send = mention_req_user + message_to_send
        message_to_send = message_to_send + "\n\n" + "#uploads"
    else:
        message_to_send = "<i>Gagal</i> mengunggah berkas😞😞"
        ...
        if not complete:
            if not file.error_message:
                ...
                try:
                    ...
                    pass
                #
                msg = f"\nMengunduh: `{downloading_dir_name}`"
                msg += f"\nKecepatan: {file.download_speed_string()} 🔽 / {file.upload_speed_string()} 🔼"
                msg += f"\nTerunduh: {file.progress_string()}"
                msg += f"\nUkuran File: {file.total_length_string()}"

                if is_file is None :
                   msg += f"\n<b>Koneksi:</b> {file.connections}"
                else :
            ...
            await event.edit(f"Berkas berhasil diunduh: `{file.name}`")
            return True
    except Exception as e:
        LOGGER.info(str(e))
        if " not found" in str(e) or "'file'" in str(e):
            await event.edit("Undihan dibatalkan:\n`{}`".format(file.name))
            return False
        elif " depth exceeded" in str(e):
            file.remove(force=True)
            await event.edit("Unduhan dibatalkan secara otomatis:\n`{}`\nLink/Torrent anda mati".format(file.name))
            return False
        else:
            LOGGER.info(str(e))
