...
async def button(bot, update: CallbackQuery):
    ...
    if cb_data.startswith("leech"):
        ...
        if dl_url is not None:
            await update.message.edit_text("mengekstrak link")
            ...
            if "_" in cb_data:
                ...
                await update.message.edit_text("mencoba mengunduh")
        ...
        i_m_sefg = await update.message.edit_text("memproses")
        # LOGGER.info(message)
        ...
        LOGGER.info(cf_name)
        if dl_url is not None:
            await i_m_sefg.edit_text("mengekstrak alamat")
            current_user_id = update.from_user.id
            # create an unique directory
