...
if __name__ == "__main__" :
    ...
    status_message_handler = MessageHandler(
        status_message_f,
        filters=Filters.command(["daftar"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    exec_message_handler = MessageHandler(
        exec_message_f,
        filters=Filters.command(["terminal"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    upload_document_handler = MessageHandler(
        upload_document_f,
        filters=Filters.command(["unggah"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    save_thumb_nail_handler = MessageHandler(
        save_thumb_nail,
        filters=Filters.command(["simpangambar"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(save_thumb_nail_handler)
    #
    clear_thumb_nail_handler = MessageHandler(
        clear_thumb_nail,
        filters=Filters.command(["hapusgambar"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
